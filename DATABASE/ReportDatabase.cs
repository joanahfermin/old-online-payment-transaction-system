    ...
    internal class ReportDatabase
    {
        ...
        public static List<ReportCollection> SelectUserRPT(DateTime _DateFrom, DateTime _DateTo)
        {
            using (SqlConnection conn = DbUtils.getConnection())
            {
                ...
                "SELECT TaxDec, TaxPayerName, Collection, Billing, ExcessShort " +
                "FROM ( " +
                " SELECT TaxDec, TaxPayerName, AmountTransferred as Collection,  AmountToPay as Billing, 0 as ExcessShort, ValidatedDate, RPTID " +
                " FROM Jo_RPT r " +
                ...
                " SELECT TaxDec, TaxPayerName, TotalAmountTransferred as Collection, AmountToPay as Billing, ExcessShortAmount as ExcessShort, (select min(ValidatedDate) from Jo_RPT r2 where r2.RefNum = r.RefNum ) as ValidatedDate, RPTID " +
                " FROM Jo_RPT r " +
                " WHERE Bank not in @OnlinePaymentChannels and RefNum is not null " +
                " and DeletedRecord = 0 and CAST(ValidatedDate AS Date)>= CAST(@FromDate AS Date) and CAST(ValidatedDate AS Date) <= CAST(@ToDate AS Date) and ValidatedBy=@UserName " +
                " UNION " +

                " SELECT TaxDec, TaxPayerName, TotalAmountTransferred as Collection, AmountToPay as Billing, ExcessShortAmount as ExcessShort, ValidatedDate, RPTID " +
                " FROM Jo_RPT r " +
